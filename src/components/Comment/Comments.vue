    ...
    <div class="innerWrapper">
      <div class="addComment" key="addComment">
        ...
        <div class="commentBox" key="commentBox" v-show="!showSignPanel && !auth || auth ">
            ...
            ></textarea>
            <button aria-label="Invia" v-if="requestLoading"><div class="requestLoading"></div></button>
            <button @click="addComment" aria-label="Invia" v-else>Invia</button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        //userId: null,
        isAdmin: null,
        //userName: this.$store.getters.user.id,
        expiresIn: null,
        auth: false,
        wrapperSize: '',
    ...
    methods: {
      checkAutoLogin() {
        ...
        //if (now >= expirationDate) {
          //return
        //}
        //const userId = localStorage.getItem('commentUserId') //this.$store.getters.user.id
        //const userName = localStorage.getItem('commentUserName')
        if(localStorage.getItem('commentAdmin') != 'undefined'){
          const data = [token, '', '', 1, true]
          this.signUser(data);
        } else {
          const data = [token, '', '', 1]
          this.signUser(data);
      ...
      addComment() {
        this.idToken = localStorage.getItem('commentToken')
        console.log(this.baseURL + '/commentsGrid/' + this.nodeName + '/comments.json' + '?auth=' + this.idToken)
        ...
        if (this.auth) {
          if(this.userName == ""){
            this.setAlert('Rieffettua il login \n', 'fail')
            this.clearAuth()
            return
          }
